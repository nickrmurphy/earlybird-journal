...
export const days = t.pgTable('days', {
    id: t.uuid().primaryKey().defaultRandom(),
    journalId: t.uuid().notNull().references(() => journals.id),
    title: t.text().notNull().default(sql`to_char(now(), 'FMDay, YYYY Month DD')`),
    date: t.date().notNull().defaultNow(),
    updatedAt: t.timestamp({ withTimezone: false }).notNull().defaultNow(),
    updatedBy: t.text().notNull(),
}, (table) => [
    t.unique().on(table.journalId, table.date).nullsNotDistinct(),
]);
...
export const actionStatusEnum = t.pgEnum('action_status', ['incomplete', 'complete', 'canceled']);
