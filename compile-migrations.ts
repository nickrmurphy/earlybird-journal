...
export async function compileMigrations(migrationsFolder: string, outputFolder: string) {
	...
	const migrations = readMigrationFiles({ migrationsFolder: absoluteMigrationsPath });
	await Bun.write(`${absoluteOutputPath}/migrations.json`, JSON.stringify(migrations));
